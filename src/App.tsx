...
function App() {
  ...
  const changeSelected = (id: number) => {
    setTodos(prev => prev.map(todo => todo.id === id ? { ...todo, selected: !todo.selected } : todo))
  }
  ...
  return (
    <div className='container mx-auto pt-6 flex gap-4 flex-col justify-center text-sm'>
      <TodoHeader todos={todos} toggleSelect={toggleSelect} createTodo={createTodo} select={select} doneSelected={doneSelected}/>
      <TodoList changeSelected={changeSelected} todos={todos} moveDown={moveDown} changeDone={changeDone} deleteToDo={deleteToDo} moveUp={moveUp} select={select}/>
    </div>
  )
}
